...
/* ────────────────────────────────────────────────
   Module Crédibilité — signaux de confiance
   ──────────────────────────────────────────────── */
...
export async function analyzeCredibility(domain: string, homepage: FetchedPage | null): Promise<DimensionResult> {
  ...
  checks.push({
    ...
    detail: hasLegal ? "Lien vers mentions légales présent" : "Aucun lien vers mentions légales",
  ...
  checks.push({
    ...
    detail: hasPrivacy ? "Lien vers privacy policy présent" : "Aucune politique de confidentialité",
  ...
  // 4. Présence sociale (au moins 1 lien vers une plateforme)
  ...
  checks.push({
    id: "social-proof",
    label: "Présence sociale",
    status: socialGood ? "pass" : socialCount === 1 ? "warn" : "fail",
    detail: `${socialCount} réseau(x) social(aux) référencé(s)`,
  ...
  checks.push({
    id: "copyright",
    label: "Copyright à jour",
    status: hasRecentCopyright ? "pass" : "warn",
    detail: hasRecentCopyright
      ? `Copyright ${currentYear} ou ${previousYear} détecté`
      : `Pas de copyright récent (attendu : ${currentYear})`,
    points: hasRecentCopyright ? 10 : 0,
    maxPoints: 10,
  });

  // 6. Témoignages / avis (signaux de preuve sociale)
  ...
  checks.push({
    ...
    detail: hasTestimonials ? "Signaux de preuve sociale détectés" : "Pas de témoignages visibles",
